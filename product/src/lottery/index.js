...
function lottery() {
  ...
  rotateBall().then(() => {
    ...
    // 获取当前奖项的内定用户配置
    const designatedIds = DESIGNATED_USERS[currentPrize.title] || [];

    // 处理内定用户逻辑
    designatedIds.forEach(userId => {
      // 确保内定用户存在于剩余用户中
      const userIndex = basicData.leftUsers.findIndex(u => u[0] === userId);
      if (userIndex === -1) return;

      // 将内定用户加入中奖名单
      currentLuckys.push(basicData.leftUsers.splice(userIndex, 1)[0]);
    ...
    // 计算剩余需要抽取的数量
    let perCount = EACH_COUNT[currentPrizeIndex] - currentLuckys.length;
    let leftCount = basicData.leftUsers.length;
    // 修改为
    const luckyUsersOfType = basicData.luckyUsers[currentPrize.type];
    let leftPrizeCount = currentPrize.count - (luckyUsersOfType ? luckyUsersOfType.length : 0) - currentLuckys.length;
    // 抽取剩余用户
    for (let i = 0; i < perCount; i++) {
      if (leftCount <= 0 || leftPrizeCount <= 0) break;
...
function changeCard(cardIndex, user) {
  ...
  card.innerHTML = `<div class="company">${COMPANY}</div><div class="name">${user[1]
    }</div><div class="details">${user[0]}<br/>${user[2] || "wedding"}</div>`;
}
